...
function wfsection_search($queryarray, $andor, $limit, $offset, $userid)
{
    ...
    if (!empty($queryarray)) {
        $sql = 'SELECT articleid,fileshowname FROM ' . $xoopsDB->prefix('zmag_files');
        ...
        while ($row = $xoopsDB->fetchArray($result)) {
            ...
            if (!in_array($row['articleid'], $articles_arr, true)) {
                $articles_arr[] = $row['articleid'];
            }
        }
    }
    ...
    $sql = 'SELECT articleid,uid,title,published FROM ' . $xoopsDB->prefix('zmag_article') . ' WHERE published>0 AND published<=' . time() . '';

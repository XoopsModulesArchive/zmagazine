...
require XOOPS_ROOT_PATH . '/modules/zmagazine/include/groupaccess.php';

$myts = MyTextSanitizer::getInstance(); // MyTextSanitizer object
$mytree = new XoopsTree($xoopsDB->prefix('zmag_category'), 'id', 'pid');
$GLOBALS['xoopsOption']['template_main'] = 'wfsection_topten.html';
...
$result = $xoopsDB->query('SELECT id, title, groupid FROM ' . $xoopsDB->prefix('zmag_category') . ' WHERE pid=0');
$e = 0;
$rankings = [];
while (list($cid, $ctitle, $groupid) = $xoopsDB->fetchRow($result)) {
    if (checkAccess($groupid)) {
        $rankings[$e]['title'] = sprintf(_WFS_TOP10, htmlspecialchars($ctitle, ENT_QUOTES | ENT_HTML5));

        $query = 'SELECT articleid, categoryid, title, counter, rating, votes, groupid FROM ' . $xoopsDB->prefix('zmag_article') . " WHERE (categoryid=$cid";
        ...
        while (list($darticleid, $dcategoryid, $dtitle, $counter, $rating, $votes, $dgroupid) = $xoopsDB->fetchRow($result2)) {
            if (checkAccess($dgroupid)) {
                ...
                $rankings[$e]['file'][] = ['id' => $darticleid, 'cid' => $dcategoryid, 'rank' => $rank, 'title' => $dtitle, 'category' => $catpath, 'hits' => $counter, 'rating' => number_format($rating, 2), 'votes' => $votes];

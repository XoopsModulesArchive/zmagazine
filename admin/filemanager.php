...
global $xoopsConfig, $xoopsModule, $wfsConfig, $dirlist, $dirpath, $_GET;
...
$filelist = [];
    ...
    if ($handle = @opendir($dirpath)) {
        ...
        while (false !== ($files = readdir($handle))) {
            if ('.' != $files && '..' != $files && '_private' != $files && 'cvs' != mb_strtolower($files)) {
    ...
    if (is_dir($dirname) && $handle = @opendir($dirname)) {
        while (false !== ($files = @readdir($handle))) {
            if (!preg_match('/^[.]{1,2}$/', $files)) {
                if ('cvs' != mb_strtolower($files) && is_dir($dirname . '/' . $files)) {
...
if ('edit' == $action) {
    if ($confirm) {
        ...
        $content = stripcslashes($myts->addSlashes($_POST['text']));
    ...
    redirect_header((string)$urlpath, 1, 'No File to Edit');
...
if ('rename' == $action) {
    ...
    $sform->addElement(new XoopsFormHidden('old_filename', basename(htmlentities($file, ENT_QUOTES | ENT_HTML5))));
...
if ('mkdir' == $action) {
    ...
    $newdir = mb_strtolower("{$_POST['workpath']}/{$newdir}");
...
if ('mkfile' == $action) {
    ...
    $newfile = mb_strtolower("{$_POST['workpath']}/{$newfile}");
...
if ('upload' == $action) {
    ...
    if (isset($_POST['imgname']) && !empty($_POST['imgname'])) {
        $imgname = mb_strtolower($_POST['imgname']);
    }
    ...
    if (empty($upload->fileName)) {
        echo 'No name choosen <br><br>';
    }

    if (!is_writable($_POST['uploadpath'])) {
        echo $workdir . _AM_ISNOTWRITABLE . ' <br><br>';
    }

    if (!is_file($upload->fileName)) {
        echo $upload->fileName . _AM_ISNOTWRITABLE . ' <br><br>';
    }

    if (!$upload->isAllowedMineType()) {
        echo _AM_NOTALLOWEDMINETYPE . '<br><br>';
    }

    $size = getimagesize($upload->fileName);

    if (!$upload->isAllowedImageSize()) {
        echo _AM_IMGESIZETOBIG . ' <br><br>Allowed image dimensions: Height:' . $wfsConfig['imgheight'] . ' x Width:' . $wfsConfig['imgwidth'] . '<br>Uploading image dimensions: Height:' . $size[0] . ' x Width:' . $size[1] . '<br><br>';
    }

    if (!$upload->isAllowedFileSize()) {
        echo _AM_FILETOOBIG . ' Maximum allowed is ' . Prettysize($wfsConfig['filesize']) . '<br><br>';
    }

    if (is_file($_POST['uploadpath'] . '/' . $upload->originalName)) {
        echo 'The file you are uploading exists on the server.<br><br>';
    }
...
echo "<table width = 100% cellspacing = '0' cellpadding = '1' class = outer >";
        ...
        echo '<br><br>';
        if (ini_get('safe_mode')) {
            echo 'safe_mode is ON (This will cause problems with Filemanger)';
        } else {
        ...
        if (ini_get('enable_dl')) {
            echo ' and Max Upload size = ' . ini_get('upload_max_filesize');
        }
        echo '<br>';
...
//echo"<table width='100%' border='0' cellspacing='1' cellpadding = '2' class = 'outer'>";
    if ($root != $dirpath) {
        echo "<tr><td width='4%' align='center'><a href='" . $urlpath . '?rootpath=' . mb_substr($dirpath, 0, mb_strrpos($dirpath, '/')) . "'><img src=" . XOOPS_URL . '/modules/' . $xoopsModule->dirname() . "/images/icon/back.gif></td><td align='left'><a href='" . $urlpath . '?rootpath=' . mb_substr($dirpath, 0, mb_strrpos($dirpath, '/')) . "'>Parent Directory</td></tr>";
    }
...
//Get file info and details
    for ($i = 0, $iMax = count($filelist); $i < $iMax; $i++) {
        ...
        echo '</tr>';
            ...
            echo "<td class='barbottom' align='left' colspan =3>" . $dircount = count($dirlist) . ' Directories /' . $filecount = count($filelist) . ' files</td>';

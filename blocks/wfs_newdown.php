...
//  ------------------------------------------------------------------------ //
require_once XOOPS_ROOT_PATH . '/modules/zmagazine/include/groupaccess.php';
...
function b_wfs_down_show($options)
{
    ...
    $sql = 'SELECT fileid, filerealname, filetext ,articleid, fileshowname, date, ext, minetype, downloadname, counter, filedescript, groupid FROM ' . $xoopsDB->prefix('zmag_files') . ' ORDER BY ' . $options[0] . ' DESC';
    ...
    while ($myrow = $xoopsDB->fetchArray($result)) {
        if (checkAccess($myrow['groupid'])) {
            ...
            $title = htmlspecialchars($myrow['fileshowname'], ENT_QUOTES | ENT_HTML5);

            if (!XOOPS_USE_MULTIBYTES) {
                if (mb_strlen($myrow['fileshowname']) >= $options[2]) {
                    $title = htmlspecialchars(mb_substr($myrow['fileshowname'], 0, ($options[2] - 1)), ENT_QUOTES | ENT_HTML5) . '...';
                }
            }

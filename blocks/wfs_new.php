...
function b_wfs_new_show($options)
{
    ...
    $sql = 'SELECT articleid, title, published, expired, counter, groupid FROM ' . $xoopsDB->prefix('zmag_article') . ' WHERE published < ' . time() . ' AND published > 0 AND (expired = 0 OR expired > ' . time() . ') AND noshowart = 0 AND offline = 0 ORDER BY ' . $options[0] . ' DESC';
